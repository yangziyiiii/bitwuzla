# Bitwuzla 优化修改说明：针对 `mul_2.btor2`
...
*   **修改**: 我注释掉了 `RewriteRule<RewriteRuleKind::BV_SHL_CONST>::_apply` 中将 `bvshl` 展开为 `concat` 的代码。这确保了优化方向是单向的（朝着更高级的 `bvshl` 算子方向优化）。

## 3. 添加额外的算术简化规则

为了进一步加速求解，针对引入 `bvshl` 后可能出现的算术表达式，我添加了两个新的重写规则：

**涉及文件**: `src/rewrite/rewrites_bv.cpp`, `src/rewrite/rewrites_bv.h`, `src/rewrite/rewriter.h`, `src/rewrite/rewriter.cpp`
...
通过将底层的位操作还原为高层的算术操作（左移），并配合针对性的代数简化规则（如 `2a - a = a`），我们成功消除了求解过程中的复杂性。

**最终结果**: `mul_2.btor2` 从超时（>10秒）变为瞬间求解（"unsatisfiable"），性能提升显著。
